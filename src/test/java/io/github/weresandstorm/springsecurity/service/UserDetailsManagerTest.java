...
public class UserDetailsManagerTest {
  ...
  public void setup() {
    userDetailsManager =
        new ConcreteUserDetailsManager(
            accountRepo, securityContextService, authenticationManager);
  }

  @Test
  public void shouldCreateUser() {
    // Given
    final Account userAccount = UserIdentityBuilder.userIdentityBuilder().build();

    // When
    userDetailsManager.createUser(userAccount);

    // Then
    verify(accountRepo).save(userAccount);
  }

  @Test(expected = IllegalArgumentException.class)
  public void shouldNotCreateUserWhenUsernameIsEmpty() {
    // Given
    final Account userAccount = UserIdentityBuilder.userIdentityBuilder().username("").build();
  ...
  public void shouldDeleteUser() {
    ...
    // Then
    verify(accountRepo).deleteByUsername(username);
  }
  ...
  public void shouldReturnTrueWhenUserExists() {
    ...
    // And
    given(accountRepo.findByUsername(username)).willReturn(Optional.of(userAccount));
  ...
  public void shouldReturnFalseWhenUserDoesNotExists() {
    ...
    // And
    given(accountRepo.findByUsername(username)).willReturn(Optional.empty());
  ...
  public void shouldLoadUserByUsernameWhenUserExists() {
    ...
    // And
    given(accountRepo.findByUsername(username))
        .willReturn(
  ...
  public void shouldChangePasswordForExitingUser() {
    ...
    verify(accountRepo).changePassword(currentPassword, newPassword, username);
  }
}
